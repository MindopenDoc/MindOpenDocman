...
function list_files($fileid_array, $userperms_obj, $dataDir, $showCheckBox = false, $rejectpage = false)
{
    ...
    foreach ($fileid_array as $fileid) {
        ...
        //anshuman code on line number 358
        $file_list_arr[] = array(
            'id' => $fileid,
            'view_link' => $view_link,
...
?>
        <form name = "browser_sort">
            ...
            <tr>
              ...
              <td nowrap rowspan="0">
                <select id="dep" name="category" onChange="loadItem(this)" width="0" size="1">
        ...
        ?>
            ...
            <td>
                <select id="cat" name="category_item" onChange="loadOrder(this)">
                    <option id="0" selected ><?php echo msg('label_empty');?></option>
                </select>
            </td>
            <td>
                <select id="opt" name="depart_cat" disabled>
                <option value="" >Select An dep-category</option>
                </select>
            </td>
            <td>
                <select id="subopt" name="depart_subcat" disabled>
                    <option value="">Select an dep-sub-category</option>
                </select>
            </td>
            <td>
                <select name="category_item_order" onChange="load1(this)">
                    <option id="0" selected ><?php echo msg('label_empty');?></option>
                </select>
            </td>
        </tr>
...
<script>
var flag=false;
const myElement = document.getElementById("dep");
const myElement2 = document.getElementById("opt");
const myElement3 = document.getElementById("cat");
const myElement4= document.getElementById("subopt");
myElement.addEventListener('change', (e) => {
if (myElement.value==="department"){
    flag=true;
}
})
...
if(flag){
    ...
    showuser(myElement3.value);
}
})
myElement2.addEventListener('change', (e) => {
if(flag){
    showuser2(myElement2.value);
}
})
myElement4.addEventListener('change', (e) => {
    console.log("changed");
})
function load1(select_box) {
    if(myElement2.value && myElement4.value){
        window.location = "search.php?submit=submit&sort_by=id&where=" + category_option + "&sort_order=" + select_box.options[select_box.selectedIndex].value + "&keyword=" + escape(category_item_option) + "&exact_phrase=on&subcat="+myElement4.value+"&dep="+myElement3.value+"&cat="+myElement2.value;
    }
    else if(myElement2.value){
        window.location = "search.php?submit=submit&sort_by=id&where=" + category_option + "&sort_order=" + select_box.options[select_box.selectedIndex].value + "&keyword=" + escape(category_item_option) + "&exact_phrase=on&dep="+myElement3.value+"&cat="+myElement2.value;
    }
    else{
        window.location = "search.php?submit=submit&sort_by=id&where=" + category_option + "&sort_order=" + select_box.options[select_box.selectedIndex].value + "&keyword=" + escape(category_item_option) + "&exact_phrase=on";
    ...
    if (data == "") {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                myElement4.innerHTML = CreateOptions;
            }
        }
...
function showuser(data){
    if (data == "") {
        ...
        xmlhttp.open("GET", "getuser2.php?q=" +data, true);
        xmlhttp.send();
    }
</script>
