    ...
    function search($where, $keyword, $exact_phrase, $case_sensitivity, $search_array)
    {
        ...
        $query_pre = "
          ...
          FROM
            ...
            {$GLOBALS['CONFIG']['db_prefix']}department as dept,
        ...
        switch ($where) {
            ...
            // d.realname LIKE "check.txt"
                // $query .= 'd.realname LIKE "check.txt"';
            ...
            case 'file_id':
                ...
                break;
            case 'keyword':
                $query .= "d.keyword $equate :keyword ";
                break;
            case 'all':
        ...
        // print_r($query_pre);
        //anshuman code start
        // if(isset($_GET['subcat']) && isset($_GET['cat'])){
        //     echo $_GET['subcat'];
        //     echo $_GET['cat'];
        // }
        ...
        if(isset($_GET['subcat']) && isset($_GET['cat'])){
            ...
            $sc=$_GET['subcat'];
            $c=$_GET['cat'];
            // $damy="SELECT * FROM odm_department WHERE name='$d'";
	        // $d_result = $pdo->prepare($damy);
            // $d_result->execute();
            // $row1 = $d_result->fetchAll();
            // $d=$row1[0][0];
            if($_GET['subcat']!=""  && $_GET['subcat']!="")
            {
            $query .= ")AND (d.dep_category='$c') AND (d.sub_category='$sc'";
            }
            // elseif(isset($_GET['cat'])){
            //     echo "elseif";
            // $query .= ") AND (d.depart='$d') AND (d.dep_category='$c'";
            // }
        }
        else if(isset($_GET['cat']) && !isset($_GET['subcat'])){
            $c=$_GET['cat'];
            $query .= ")AND (d.dep_category='$c'";
