             ...
             {if $lastversion==""}
                 <td align="left">
                     <span style="font-size: larger; ">
                         <a href="{$baseurl}/view_file.php?submit=view&id={$file_id}&mimetype=text%2Fplain"
                             target="_blank">{$file_detail.realname}</a>
                     </span>
                 </td>
             {else}
                 {if $pdf2 eq "tr2"}
                     <td align="left">
                         <span style="font-size: larger; "><a href="versionfile/{$latestfile}"
                                 target="_blank">{$latestfile}</a></span>
                     </td>
                 {else}
                     <td align="left">
                         <span style="font-size: larger; "><a href="versionfile/{$latestfile}" download>{$latestfile}</a></span>
                     </td>
                 {/if}
             {/if}
         </tr>
         ...
         {if $file_detail.status gt 0}
             <tr>
                 ...
                 <td><a
                         ...
                         {$checkout_person_full_name[1]|escape:'html'}, {$checkout_person_full_name[0]|escape:'html'}</a>
                 </td>
             </tr>
         {/if}
         <!--anshuman code start -->
             ...
             {foreach from=$version_list item=foo}
                 <tr>
                     ...
                     <td style="text-align: center;">
                         {if $pdf1 eq "tr1"}
                             <button><a href="versionfile/{$foo.filename}" target="_blank">{"view"}</a></button>
                         </td>
                     {else}
                         <button><a href="versionfile/{$foo.filename}" download>{"Download"}</a></button></td>
                     {/if}
                 </tr>
             {/foreach}
         </table>
         ...
         <td colspan="2">
             <table border="1" width=100% style="margin:2rem 0rem" cellspacing="4" cellpadding="1">
                 ...
                 <tr>
                     ...
                     <td style="text-align: center;"> <a class="btn negative" href="{$history_link|escape}"
                             target="_blank">{"view logs"}</a></td>
                 </tr>
             </table>
         </td>
     </tr>
             ...
             <table border="0" cellspacing="5" cellpadding="5">
                 <tr>
                     ...
                     {if $File_Owner_bool or $isadmin eq 'yes' }
                         <td colspan="2" align="right"><a href="{$File_permission_alter|escape}"><button class="negative">
                                     Edit permission</button></a> </td>
 ...
 <div class="container">
     <form id="addComment" action="addComment_V.php" name="addComment" method="POST" onsubmit="return checksec();">
         <table width=100% cellspacing="4" cellpadding="1">
             ...
             <tr>
                 <td colspan="2"> <input class="positive" style="float:right;" type="submit" value="Add Comment"
                         name="Comment" /></td>
             </tr>
         </table>
     </form>
